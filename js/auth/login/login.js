    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Busca permissão do usuário na tabela grupo
                let permissao = null;
                try {
                    permissao = await window.buscarPermissaoUsuario(username);
                } catch (err) {
                    console.warn('[Login] Não foi possível buscar permissão:', err);
                }

                // Salva sessão + permissão
                const lembrar = document.getElementById('rememberMe')?.checked || false;
                window.salvarSessao(users, lembrar, permissao);
                ...
                console.log('[Login] Usuário autenticado:', users.nome, 
                            permissao ? '(nível ' + permissao.nivel + ')' : '(sem permissão)');
